    ...
    public function create (Request $request)
    {
        ...
        $amount=intval($request->amount); //string to number
         ...
         if($amount < 5000)
         {
            ...
            return Redirect::to('/gsmile/create')->with('notification','Ops! Minimum amount is 5,000 Naira')->withInput();
         }
         ...
         if($amount > 200000)
         {
            ...
            return Redirect::to('/gsmile/create')->with('notification','Ops! Maximum amount is 200,000 Naira')->withInput();
         }
        ...
        if($userFirstGsAmount > $request->amount){
            ...
            return Redirect::to('/gsmile/create')->with('notification','Your new SL amount can not be lesser than your last SL amount ('.number_format($userFirstGsAmount,2).' Naira)')->withInput();
        }
        ...
        $achievedSmiles=Gsmile::where('user_id', Auth::user()->id)->orderBy('id','asc')->get();
                ...
                if(Auth::user()->is_pioneer <=0){

                if(in_array(0, $leftAmountStatusArray) && Auth::user()->is_pioneer <= 0)
                {

                     $not="You must fufill all previous SL requests before new SL request";
                  ...
                  if(in_array(0, $leftAmountStatusArray) && Auth::user()->is_pioneer == 0)
                {
                 ...
                 if(Auth::user()->is_pioneer >= 1){
                    ...
                    $countFalseValuesArray = array_count_values($leftAmountStatusArray);

                    $countFalseValuesArrayFlip =array_flip($countFalseValuesArray);
                    //echo json_encode($countFalseValuesArray);

                    //echo $d;

                    if(in_array(0, $leftAmountStatusArray)){
                         $countFalseValues = $countFalseValuesArray['0'];

                         if($countFalseValues > 1){

                            $not="Pioneer can only have three SL requests running concurrently.";
...
//echo $countFalseValues;
                 ...
                 if(Auth::user()->is_pioneer == '1')
                 {
                    ...
                    $formData=$request->all();
                        ...
                        $message=array(
                            'amount.required'=>'Laughter Amount is required.',
                            );
                            ...
                            //
                                if(collect($AllGsmilesForPioneer)->isEmpty()){
                                 ...
                                 // match 100% for pioneer
                                    ...
                                    return Redirect::to('/gsmile/create')->with('notification','You have succesfully created a laughter request. You have been matched with '.$response['pioneer_name'].'('.$response['pioneer_phone'].') to make  payment of '.$response['insuranceAmount'].' Naira (100%). Check Matchlist table for account details.');
                                            ...
                                            if(isset($request->retainment_id)){
                                                ...
                                                if($rid < $rToPay){
                                                    return Redirect::to('/wallet/view')->with('You must make new SL of 100% of retainment capital.');
                                                }
                                               ...
                                               //$parseDetails=['retainment_id']=$request->re
                                                ...
                                                return Redirect::to('/wallet/view')->with('Already SL to unlock retained smile.');
                                            }
                                            ...
                                            return Redirect::to('/gsmile/create')->with('notification','You have succesfully created a laughter request. You have been matched with '.$response['pioneer_name'].'('.$response['pioneer_phone'].') to make insurance payment of '.$response['insuranceAmount'].' Naira (30%). Check Matchlist table for account details.');
                    ...
                    return Redirect::to('/gsmile/create')->with('notification','You have succesfully created a laughter request. You have been matched with '.$response['pioneer_name'].'('.$response['pioneer_phone'].') to make insurance payement of '.$response['insuranceAmount'].' Naira (30%). Check Matchlist table for account details.');
    ...
    public function saveInitPioneerGsRequest($request)
{
    ...
    //Create Virtual GS request for bot
         ...
         $createSmile1->user_id=User::where('email', 'system@gmail.com')->first()->id;
    ...
    //create virtual RS for bot
         ...
         $rsmile->user_id=User::where('email', 'system@gmail.com')->first()->id;
        ...
        //match user 100% with central bot
         ...
         $match->rsmile_user_id=User::where('email', 'system@gmail.com')->first()->id;
         ...
         $bot=User::where('email', 'system@gmail.com')->first();
         ...
         $bonus->referee_gsmile_id=$pioneer_gsmile_id;
         ...
         if($referer_user->id == 101 || $referer_user->is_teamlead == '1'){
         ...
         $bonus1->referee_gsmile_id=$pioneer_gsmile_id;
         ...
         $bonus2->referee_gsmile_id=$pioneer_gsmile_id;
    ...
    if(isset($request['retainment_id'])){
        ...
        //create GS request for req 'owner'
         ...
         if(collect($bg)->isEmpty()){
            $insuranceAmount=floor((30/100)*$request['amount']);
         }else{
                ...
                if(collect($achievedGsmilesByPioneer)->isEmpty()){
                   ...
                   // echo $randomNumber;
                    if(collect($lastRsmilesOfPioneer)->isEmpty()){
                       ...
                       // echo json_encode($lastRsmilesOfPioneer);
...
//echo $pioneer_id;
                          ...
                          if($pioneer_id==Auth::user()->id){
                            ...
                            //envisage negative new left amount if matched to pioneer and flashed the admin to do do the rest matching

                        $left_amount_of_pioneer_rs=$pioneer_rsmile->left_amount;
        $new_rsLeftAmount=$left_amount_of_pioneer_rs - $insuranceAmount;

                        if($new_rsLeftAmount<0){

                            //flash the admin

                            $messageAdmin=new Message;
                            $messageAdmin->type=3;
                            $messageAdmin->user_id=0;
                            $messageAdmin->name="System";
                            $messageAdmin->email="admin@laughtercommunity.com";
                            $messageAdmin->title="Insurance matching difficulty";
                            $messageAdmin->body="Unable to matching remaining smile of pioneer. Name:".$pioneer->name." Reap laughter value=".$pioneer_rsmile->amount." Date Create =".$pioneer_rsmile->created_at.". Help match him.";
                            $messageAdmin->attachment_link=0;
...
//referrer section
         ...
         $bonus->referee_gsmile_id=$owner_gsmile_id;
         ...
         $bonus1->referee_gsmile_id=$owner_gsmile_id;
         ...
         $bonus2->referee_gsmile_id=$owner_gsmile_id;
        ...
        break;
                ...
                }// closes if no pioneer  hasmatch is set
        ...
        //Create Virtual GS request for bot2
         ...
         $createSmile1->user_id=User::where('email', 'system2@gmail.com')->first()->id;
    ...
    //create virtual RS for bot2
         ...
         $rsmile->user_id=User::where('email', 'system2@gmail.com')->first()->id;
        ...
        //match user 20% with  bot2
         ...
         $match->rsmile_user_id=User::where('email', 'system2@gmail.com')->first()->id;
         ...
         $bot=User::where('email', 'system2@gmail.com')->first();
