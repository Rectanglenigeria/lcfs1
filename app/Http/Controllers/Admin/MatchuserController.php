...
if(!empty($matches)){
    ...
    public function MatchManually(Request $request)
    {
    	...
    	$rsmile=Rsmile::where('id',$rsmile_id)->first();
			...
			$gsmileLeftAmt=$gsmile->left_amount;
       ...
       //send sms
			 ...
			 //send GS MATCHING SMS
                    ...
                    $apikey = "93b7beb42956897243858e7587df11527c212deb";
			               ...
			               $message = number_format($amountToRemoveFromGS)." of 70% laughter has been matched.Kindly make payment and get confirmed. If fully matched, proceed to have your new SL IP confirmed to enable your RL";
			               $flash = 0;
        ...
        $apikey = "93b7beb42956897243858e7587df11527c212deb";
			   ...
			   $message = "Hi Laughterian, your RL request has been matched. kindly confirm all payments received on time & proceed 2 write ur testimony. Share the good news. ";
			   $flash = 0;
			   if (get_magic_quotes_gpc()) {
			       $message = stripslashes($message);
			   }
     ...
     // $match= Matchuser::where('id', $request->matchId)->first();
      ...
      if($check == $match->amount){
                ...
                $apikey = null;
                    ...
                    $message = "Your 30% down payment of ".(number_format(0.3*$match->gsmile->amount,2))." has been confirmed. You remaining 70% will be matched within the next 10 days.";
                    $flash = 0;
                    if (get_magic_quotes_gpc()) {
                        $message = stripslashes($message);
                    }
    ...
    if($check2 == $match->amount){
              ...
              $apikey = null;
                  ...
                  $recipients = $match->gsmile_user->phone;
                  $message = "Your 70% down payment of ".(number_format(0.7*$match->gsmile->amount,2))." has been confirmed. You remaining 70% will be matched within the next 10 days.";
                  $flash = 0;
                  if (get_magic_quotes_gpc()) {
                      $message = stripslashes($message);
                  }
