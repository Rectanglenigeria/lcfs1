...
class MatchuserController extends Controller
{
    ...
    public function extendPaymentTime($match_id)
    {
    	...
    	$timeUsed=intval(time())-intval(strtotime($match->created_at));
    	if($match->is_extended == 1){
		  	$allocatedTime=(48+8)*60*60 ;  //48 + 6 hours
		  }else{
		  	 $allocatedTime=48*60*60 ; //48 hours to seconds
		  }
     ...
     // $match= Matchuser::where('id', $request->matchId)->first();
      ...
      if($check == $match->amount){
                ...
                $apikey = null;
                    ...
                    $message = "Your 20% down payment of ".(number_format(0.1*$match->gsmile->amount,2))." has been confirmed. You remaining 80% will be matched within the next 15 days. Keep smilling!";
                    $flash = 0;
